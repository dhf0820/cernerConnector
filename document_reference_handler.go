...
// 	if err != nil {
// 		fmt.Printf("findResource:400  --  ReadAll FhirSystem error %s\n", err.Error())
// 		errMsg := err.Error()
...
// 	if err != nil {
// 		fmt.Printf("\nfindResource:410  --  unmarshal err = %s\n", err.Error())
// 		errMsg := err.Error()
// 		WriteFhirOperationOutcome(w, 400, CreateOperationOutcome(fhir.IssueTypeProcessing, fhir.IssueSeverityFatal, &errMsg))
// 		return
// 	}
...
// 	resource, err := GetResource(&connectorPayload, Resource, uri, JWToken)
...
// 	url := fmt.Sprintf("%s/%s%s", sysCfg.Url, Resource, uri) //" + "/" + uri
