...
func TestObservationSearch(t *testing.T) {
	log.SetDebuglevel("DEBUG3")
	log.Debug3("TestObservationSearch")
	...
	//c := New(baseurl)
	Convey("RunObservationQuery", t, func() {
		...
		cnt, bundle, header, err := FindObservation(cp, "dharman0127", "patient=12724066&_count=20", jwt)
		//bundle, err := c.PatientSearch(fhirSystem, "family=smart&_count=12", "patient", newToken)
		So(header, ShouldNotBeNil)
		So(cnt, ShouldNotEqual, 0)
		So(err, ShouldBeNil)
		So(bundle, ShouldNotBeNil)
		log.Debug3(fmt.Sprintf("returned %d resources", cnt))
		//fmt.Printf("PatientSearch returned: %s\n", spew.Sdump(bundle))
		// data, err := c.Query("Patient/12724066")
		// So(err, ShouldBeNil)
		// So(data, ShouldNotBeNil)
		obs, err := fhir.UnmarshalObservation(bundle.Entry[0].Resource)
		So(err, ShouldBeNil)
		So(obs, ShouldNotBeNil)

		//fmt.Printf("PatientSearch[0] returned: %s\n", spew.Sdump(pat))
		log.Debug3("Observation.ID = " + *obs.Id)
		time.Sleep(15 * time.Second)
	})
}
